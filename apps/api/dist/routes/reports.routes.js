...
reportsRouter.post('/', requirePermission('reports:generate'), validateBody(generateReportSchema), (req, res) => {
    const report = reportService.generateReport(req.user.id, req.body);
    res.status(201).json({ data: report });
});
reportsRouter.get('/:id/export', requirePermission('reports:export'), (req, res) => {
    const format = req.query.format ?? 'json';
    const content = reportService.exportReport(req.user.id, paramId(req), format);
    if (!content) {
