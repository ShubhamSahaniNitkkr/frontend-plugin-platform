...
pluginsRouter.post(
  ...
  (req, res) => {
    try {
      const plugin = registryService.installPlugin(
        req.user!.id,
        paramId(req),
        req.body.version
      );
...
pluginsRouter.post(
  ...
  (req, res) => {
    try {
      const plugin = registryService.updatePlugin(
        req.user!.id,
        paramId(req),
        req.body.version
      );
...
pluginsRouter.put(
  '/:id/config',
  validateBody(pluginConfigSchema),
  (req, res) => {
    const config = registryService.setPluginConfig(
      req.user!.id,
      paramId(req),
      req.body
    );
...
pluginsRouter.patch(
  ...
  validateBody(pluginConfigSchema),
  (req, res) => {
    const config = registryService.patchPluginConfig(
      req.user!.id,
      paramId(req),
      req.body
    );
...
pluginsRouter.patch('/:id/health', (req, res) => {
  const { health, lastError, loadTimeMs } = req.body as {
    health: string;
    lastError?: string;
    loadTimeMs?: number;
  };
  registryService.updatePluginHealth(
    req.user!.id,
    paramId(req),
    health,
    lastError,
